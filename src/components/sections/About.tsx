...
`}<span className="text-white/40">{`//    audit/listeners      → loga ação no audit trail`}</span>
  ...
  },
  {
    ...
    render: () => (
      <>
<span className="text-white/40">{`// Nixpacks (Railway) NÃO roda prisma migrate deploy · solução: SQL no boot`}</span>{`
`}<span className="text-indigo-400">async function</span>{` `}<span className="text-cyan-400">ensureSchemaSync</span>{`() {
  `}<span className="text-white/40">{`// Cada SQL individual · múltiplos num call = erro 42601`}</span>{`
  `}<span className="text-indigo-400">await</span>{` prisma.`}<span className="text-fuchsia-400">$executeRawUnsafe</span>{`(
    `}<span className="text-fuchsia-400">{`\`ALTER TABLE "ContentAsset" ADD COLUMN IF NOT EXISTS "blocks" JSONB\``}</span>{`
  ).`}<span className="text-indigo-400">catch</span>{`(() =&gt; {});

  `}<span className="text-indigo-400">await</span>{` prisma.`}<span className="text-fuchsia-400">$executeRawUnsafe</span>{`(
    `}<span className="text-fuchsia-400">{`\`ALTER TYPE "AdminTaskStatus" ADD VALUE IF NOT EXISTS 'APPROVED'\``}</span>{`
  ).`}<span className="text-indigo-400">catch</span>{`(() =&gt; {});

  `}<span className="text-white/40">{`// ... 90 statements idempotentes · IF NOT EXISTS = NOOP · .catch = não bloqueia`}</span>{`
}
            ...
            >
              <Icon
                className={`h-10 w-10 transition-colors duration-150 ${
                  isActive
                    ? "about-tab-icon-active text-[#4F46E5] dark:text-[#818CF8]"
                    : "about-tab-icon-inactive text-gray-400 dark:text-gray-500"
                }`}
              />
              <span
                className={`text-sm font-semibold ${
                  isActive
                    ? "about-tab-label-active text-[#4F46E5] dark:text-white"
                    : "about-tab-label-inactive text-gray-600 dark:text-gray-400"
                }`}
              >
                {t(tab.labelKey)}
              </span>
